...
The `interactive` subcommand is available as `exapump interactive`. Connection arguments (`--dsn` / `EXAPUMP_DSN`) are provided via the shared `ConnectionArgs` flattened into the command's args.
...
* *AND* stderr MUST indicate the DSN is required
