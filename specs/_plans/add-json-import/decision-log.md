...
- **Rationale:** `build_sql_schema` emits plain `CREATE TABLE` plus `ALTER TABLE ... ADD CONSTRAINT`, so a second run against the same target fails on both. exapump's CSV and Parquet uploads are re-runnable, and `src/commands/upload.rs` already applies the same `CREATE TABLE IF NOT EXISTS` rewrite. Upstream emits the constraints `DISABLE`, so they enforce nothing and serve as relationship metadata only. `TableBuffer` restarts `_id` at 1 on every run (`json_tables_core/src/buffer.rs:114`), so an `_id` value is unique only within one run and a `_parent` value resolves only within the run that wrote it. Swallowing errors and parsing error strings are both worse than leaving the metadata out. Applying constraints only to newly created tables stays a follow-up.
- **Consequence:** Every JSON import prints a warning on stderr. The warning states that `_id` values repeat across runs and that the family linkage holds only within one run.
- **Promotes to ADR:** no
...
### [open-questions] The partial-load report had no carrier through the `load` interface

- **Finding:** `[UNSTATED_ASSUMPTION]` BLOCKER, round 2. `load` returned `anyhow::Result<Vec<(String, u64)>>`, whose `Err` arm carries only an `anyhow::Error`. Task 2.7 promised to preserve the per-table counts inside that error. Task 2.8 promised to print them from `src/commands/upload.rs`, which receives only the error. The spec step "stdout MUST list the tables loaded before the failure" was therefore not implementable, and `partial_family_failure_reports_loaded_tables` could not be written. The three mechanisms an implementer could invent include two that move stdout writes into `src/json_tables.rs`, which plan.md § Patterns keeps free of output.
- **Direction change:** `load` now returns `(Vec<(String, u64)>, Option<anyhow::Error>)`. The vector holds every table loaded before the error, in family order, and every table of the family on success. Task 2.7 returns that pair instead of preserving counts inside an error. Task 2.8 prints the vector on the success path and on the failure path, and every stdout write stays in `src/commands/upload.rs`.
- **Human input:** The plan was blocked on this finding. The human answered the open-questions quiz and approved the reviewer's proposed fix as-is, with no changes requested.
- **Promotes to ADR:** no

### [open-questions] `_id` restarts at 1 on every run, so the family linkage is per-run

- **Finding:** `[UNSTATED_ASSUMPTION]` BLOCKER, round 2. Decision [7] justified dropping the constraint statements by claiming the `_id`, `_parent`, `_pos`, and `<name>|object` columns still carry the relationship. `TableBuffer::new` sets `next_id: 1` (`json_tables_core/src/buffer.rs:114`), so every run restarts the counter. A repeated run writes a second root row with `_id = 1`, and every `_parent` value in the subtables then matches two parent rows. The scenario "Repeated run loads into the existing family" required that run to succeed and documented nothing about the effect.
- **Direction change:** Decision [7] § Rationale now states the upstream fact in place of the linkage claim, and carries the stderr warning as a Consequence. The `upload/json-import` spec Background carries the same fact as a named limit. The scenario "Repeated run loads into the existing family" now requires a stderr warning that `_id` values repeat across runs and that the family linkage holds only within one run. The warning is unconditional, because the command does not probe the catalog for a pre-existing family. Task 2.8 prints it, and plan.md § Verification asserts it in `repeated_run_appends_to_existing_family`.
- **Human input:** The plan was blocked on this finding. The human answered the open-questions quiz and approved the reviewer's proposed fix as-is, with no changes requested.
- **Promotes to ADR:** no
