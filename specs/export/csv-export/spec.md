...
Export an Exasol table or SQL query result to a local CSV file, with configurable formatting options. The export uses exarrow-rs HTTP transport to stream data from Exasol directly to disk. File splitting is covered by `export/csv-export-splitting`; the client-side export deadline is covered by `export/csv-export-timeout`.
